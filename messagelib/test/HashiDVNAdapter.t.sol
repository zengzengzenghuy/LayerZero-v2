...
contract HashiDVNAdapterTest is Test {
    ...
    function test_assignJob() public {
        ...
        Packet memory packet = Packet(
            ...
            alice, // sender
            dstEid,
            bytes32(uint256(uint160(bob))), // receiver
            guid,
            message
        );
    ...
    function test_verifyMessageHash() public {
        ...
        // Call storeHash from Hashi's message relay adapters
        ...
        vm.expectEmit(true, true, false, false, address(hashiDVNAdapter));
        emit HashiDVNAdapter.HashFromAdaptersMatched(
            0, // messageId
            bytes32(
                0x17edeea6237bc72c7ad210822016452e8f36661abae691770270c8f21a75cd18
            ) // message Hash from Hashi Adapters
        );
        hashiDVNAdapter.verifyMessageHash(messageId, payload);
    }

    function test_Revert_verifyMessageHash_onlyOwner() public {
        // Generate Message
        bytes memory message = "";
        bytes32 guid = bytes32("0x100");
        Packet memory packet = Packet(
